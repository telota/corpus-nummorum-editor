...
>
    <template v-slot:content>
        ...
        <itemTabs
            ...
            v-on:select="(emit) => { tab = emit }"
            v-on:show-imported="(emit) => { show_imported = emit }"
            :class="$vuetify.breakpoint.mdAndUp ? 'pr-5' : 'mb-5'"
            :style="$vuetify.breakpoint.mdAndUp ? 'position:absolute; width: 175px' : ''"
        ></itemTabs>
        ...
        <div :style="$vuetify.breakpoint.mdAndUp ? 'padding-left: 175px' : ''">
            <v-card tile min-height="400" class="grey_trip pa-4" style="width: 100%">
                ...
                <div v-if="entity === 'coins' && tab === 'images'" style="width:100%">
                    ...
                    <!-- Content -->
                    <div v-for="(iterator, i) in item.images" :key="i">
                        <div class="mb-5" :class="wraped + ' align-center'">
                            <!-- Index -->
                            <div
                                class="body-1 font-weight-bold mr-1"
                                v-text="(i + 1) + '. ' + labels['imageset']"
                            ></div>
                            <v-btn
                                icon
                    ...
                    <!-- Image Links -->
                    <v-dialog v-model="link_dialog.active" persistent max-width="600">
                        <v-card tile>

                            <!-- System Header -->
                            <dialogbartop
                                icon="link"
                                :text="labels['link'] + ': ' + 'External image link'"
                ...
                <div v-else-if="tab === 'coins' || tab === 'types'" style="width: 100%">
                ...
                <!-- Basics ---------------------------------------- -------------------------------------------- -->
                <div v-else-if="tab === 'basics'" style="width: 100%">
                ...
                <div v-else-if="tab === 'obverse' || tab === 'reverse'" style="width: 100%" >
                        ...
                        <v-row>
                            <v-col cols="12" sm="12" v-for="record in ['legend', 'design']" :key="record" class="mt-n5">
                                <div :class="wraped">
                                    ...
                                    <inheritButton
                                        v-if="entity === 'coins'"
                                        :disabled="!inherited.id_type"
                                        :inherited="inherited[tab.slice(0, 1) + '_' + record]"
                                        v-on:click="confirmInheritance(tab.slice(0, 1) + '_' + record)"
                                    ></inheritButton>
                                </div>
                            </v-col>
                            ...
                            <v-col v-if="entity === 'coins'" cols="12" sm="6" class="mt-n5">
                                <InputForeignKey
                                    ...
                                    :conditions="[{ side: tab.slice(0, 1) }]"
                                    v-on:select="(emit) => { item[tab.slice(0, 1)+'_die'] = emit }"
                                ></InputForeignKey>
                            </v-col>
                        </v-row>
                    </div>
                    ...
                    <div v-for="record in ['symbols', 'monograms']" :key="record" class="mb-2">
                        ...
                        <subheader
                            ...
                            class="mb-5"
                            v-on:add="AddRelation((tab.slice(0, 1) + '_' + record), {id: null, position: null, side: tab.slice(0, 1) === 'o' ? 0 : 1})"
                            v-on:inherit="confirmInheritance(tab.slice(0, 1) + '_' + record)"
                        ></subheader>
                        ...
                        <div v-for="(iterator, i) in item[tab.slice(0, 1) + '_' + record]" :key="i" class="mt-n3">
                            ...
                            <v-row v-if="!edit_relations[tab.slice(0, 1) + '_' + record][i]">
                                <v-col cols="12" sm="8">
                                    <InputForeignKey
                                        ...
                                        :inherited="inherited[tab.slice(0, 1) + '_' + record] === 1"
                                        style="width: 100%"
                                        v-on:select="(emit) => { item[tab.slice(0, 1) + '_' + record][i].id = emit }"
                                    ></InputForeignKey>
                                </v-col>
                            ...
                            <div v-else class="d-flex component-wrap align-start mb-7 mt-3">
                                ...
                                ></div>
                                <v-btn icon
                                    :disabled="inherited[tab.slice(0, 1) + '_' + record] === 1 || !edit_relations[tab.slice(0, 1) + '_' + record][i]"
                                    @click="EditRelation(tab.slice(0, 1) + '_' + record, i)"
                                >
                                    <v-icon>edit</v-icon>
                                </v-btn>
                        ...
                        <div v-for="(iterator, i) in item[tab.slice(0, 1) + '_controlmarks']" :key="i" class="mt-n3">
                            <v-row v-if="!edit_relations[tab.slice(0, 1) + '_controlmarks'][i]">
                                ...
                                <v-col cols="12" sm="4">
                                    <div class="d-flex component-wrap">
                                        <v-text-field dense filled outlined clearable
                                            v-model="item[tab.slice(0, 1) + '_controlmarks'][i].count"
                            ...
                            <div v-else class="d-flex component-wrap align-start mb-7 mt-3">
                                ...
                                ></div>
                                <v-btn icon
                                    :disabled="!edit_relations[tab.slice(0, 1) + '_controlmarks'][i]"
                                    @click="EditRelation(tab.slice(0, 1) + '_controlmarks', i)"
                                >
                                    <v-icon>edit</v-icon>
                                </v-btn>
                                <v-btn icon class="ml-3"
                    ...
                    <div v-if="entity === 'coins'">
                        <div v-for="(record) in ['countermark', 'undertype']" :key="record">
                            <subheader :label="labels[tab] + ' ' + labels[record]" class="mb-2"></subheader>
                ...
                <!-- References ----------------------------------------- -------------------------------------------- -->
                <div v-else-if="tab === 'references'" style="width: 100%">
                                ...
                                <v-row v-if="!edit_relations.links[i]">
                                    ...
                                    <v-col cols="12" sm="8">
                                        <v-text-field dense outlined filled clearable
                                            ...
                                            label="URL"
                                            :rules="[rules.link]"
                                            @click:prepend="$root.openInNewTab(item.links[i].link)"
                                        ></v-text-field>
                                    </v-col>
                ...
                <!-- Miscellaneous ----------------------------------------- -------------------------------------------- -->
                <div v-else-if="tab === 'miscellaneous'" style="width: 100%">
                    <!-- Remarks -->
                    <div>
                        <!-- Header -->
            ...
            <v-expand-transition>
                <v-card v-if="show_imported" class="grey_trip mt-4" tile style="width: 100%">
                    <v-card-text>
                        <imported :data="item_data.dbi ? item_data.dbi.submitted : {}"></imported>
                    </v-card-text>
                </v-card>
            </v-expand-transition>
        </div>
...
import itemTabs         from './modules/itemTabs.vue'
